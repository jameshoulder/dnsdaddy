...
- `dnsdaddy-lab` no longer defaults to `127.0.0.1:53`. On a host running DNS
  ...
  upstream. The default is now the lab resolver's port, and targeting port 53
  prints a warning.
...
- **Corrected the blocklist capacity figures**, which understated memory use by
  ...
  `docs/threat-intel.md`. Measured: **180 bytes per domain, 87 MB for 500,000**.
  ...
  250-byte ceiling, so the published number now has a source of truth.

  Container limits (`GOMEMLIMIT=512MiB`, `mem_limit: 640m`) remain correct — a
  full deployment lands near 130 MB steady state. Only the documentation was
  wrong.
