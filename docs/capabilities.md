...
| First-seen domain index | Records which registered domains (eTLD+1) this installation has ever been asked about, independently of query-log retention, so "never seen before" survives a prune. One row per domain ever seen, not one per query. **Observe only** — it cannot change an answer, an RCODE, or a block decision, and there is no enforce door. Bounded three ways: a row ceiling with stalest-first eviction, a per-minute budget on new rows, and a queue that drops rather than delaying a lookup. Installation-wide rather than per network. Exposed at `GET /api/v1/first-seen` and `/first-seen/recent`, and it is what [hunt 5](threat-hunting/README.md#hunt-5--newly-observed-domains) now runs on. Past the ceiling, a first-seen time may be when counting restarted rather than the first sighting; the API says which records are unambiguous and `dnsdaddy doctor` warns at 80%. See [algorithms.md](algorithms.md#first-seen-index). |
| DNS rebinding answer filter | Private, loopback, link-local, ULA, CGNAT and unspecified addresses are removed from answers before they reach a client, including the address hints on SVCB and HTTPS records. An answer left with no address becomes NODATA by default (NXDOMAIN and REFUSED are available). Exemptions are per policy, so split-horizon names keep working. **On for fresh installs; off on upgrade until switched on**, because withholding addresses an installation has been serving is a change to that network. The filter runs on every serve including cache hits, so two networks with different exemptions never see each other's view. Addresses inside non-address records — a TXT record naming one, an SRV target — are not inspected. See [algorithms.md](algorithms.md#rebinding-filter). |
...
| Findings pushed to a notification address | Off unless `detection.webhook.url` is set, and normal to leave off. POSTs each behavioural finding to an operator-chosen endpoint — Slack, Teams, anything that accepts JSON — as exactly the document the NDJSON file holds, so no second parser is needed. A copy, never the record: the finding is stored before the notification is attempted, a bounded queue drops rather than making detection wait, and at-least-once delivery means a consumer de-duplicates on `id`. HTTPS only, to a publicly routable address, no redirects followed; an address on this machine, your own network, or a cloud metadata service is refused at startup with the reason. |
...
| Listing dates on a block | A record says how long the feed behind it had been listing the domain: first seen, last confirmed, and an expiry where the feed publishes one. Kept per `(feed, domain)`, so two feeds listing the same name keep two histories. A domain a feed drops and re-lists keeps its original first-seen. Copied onto the decision when it happens, so the explanation does not change when the feeds do. An expired listing cannot block. Bounded by machine size; past the ceiling, history for dropped domains is discarded before anything a feed still carries. `listingState` says `dated` or `unknown` — never a date the resolver guessed. |
...
| **Listing dates for anything blocked before this release** | Dates are recorded when a feed refreshes, so an installation that has been blocking for a year starts with none for what it already blocks — they appear for each feed at its next update, and a domain's first-seen is that date rather than the real one. Nothing can recover the earlier date: the feeds do not publish it. Those blocks report `listingState: unknown`, which is the honest answer rather than a plausible one. |
...
| **Syslog sink and per-vendor clients** | The NDJSON file plus a log shipper covers the same ground without a bespoke client per platform, and the webhook covers getting an alert in front of a person. A syslog emitter, CEF/LEEF formatting, or a Slack-specific message format would each be a format to version and keep in step for no capability that is missing. |
